...
#=%= less than 2 is pending
    pending_abstracts = Abstract.objects.filter(event=event, is_deleted=False,)# evalution_set__isnull=True)
    evaluated_abstracts = Abstract.objects.filter(event=event, is_deleted=False,)# evalution_set__isnull=False)
