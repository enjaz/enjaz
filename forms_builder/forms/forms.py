...
class EntriesForm(forms.Form):
    ...
    def __init__(self, form, request, formentry_model=FormEntry,
        ...
        self.fields["%s_to" % field_key] = forms.DateField(
            label=_("and"), widget=SelectDateWidget(), required=False)
        # Add ``FormEntry.form.submitter`` fields specified in ``submitter_fields``
        self.user_fields = []
        for field in self.submitter_fields:
            field_id = -1 * (len(self.user_fields) + 1)
            self.user_fields.append(field_id)
            field_key = "field_%s" % field_id
            label = field.split(".")[-1]
            self.fields["%s_export" % field_key] = forms.BooleanField(
                initial=True, label=label, required=False)
            self.fields["%s_filter" % field_key] = text_filter_field
            self.fields["%s_contains" % field_key] = forms.CharField(label=" ", required=False)

    def __iter__(self):
        """
        Yield pairs of include checkbox / filters for each field.
        """
        for field_id in [f.id for f in self.form_fields] + [0] + self.user_fields:
    ...
    def columns(self):
        ...
        # Add columns for user data
        fields.extend(
            [field.split(".")[-1] for idx, field in enumerate(self.submitter_fields)
             if self.posted_data("field_%s_export" % self.user_fields[idx])]
        )
    ...
    def rows(self, csv=False):
        ...
        # Add columns for selected user fields
        user_fields_idx = len(field_indexes) # the starting column for user fields
        selected_user_fields = [field for idx, field in enumerate(self.submitter_fields)
                                if self.posted_data("field_%s_export" % self.user_fields[idx])]
        num_columns += len(selected_user_fields)
        ...
        for field_entry in field_entries:
            if field_entry.entry_id != current_entry:
                ...
                valid_row = True
                # Add and filter by user fields
                for plus, field in enumerate(selected_user_fields):
                    t = Template("{{ " + field + " }}")
                    c = Context({"user": field_entry.entry.submitter})
                    field_value = t.render(c)
                    field_id = self.user_fields[self.submitter_fields.index(field)]
                    ...
                    if filter_args:
                        ...
                        if not filter_func(*filter_args):
                            valid_row = False
                    current_row[user_fields_idx + plus] = t.render(c)
                if include_entry_time:
            ...
            if field_entry.value and field_id in file_field_ids:
                url = reverse("forms:form_file", args=(getattr(self.form.content_object, 'id', None), field_entry.id,),
