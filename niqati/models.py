...
class Code_Collection(models.Model): # group of codes that are (1) of the same type & (2) of the same Code_Order
    ...
    def __unicode__(self):
        return self.parent_order.episode.activity.name + " - " + self.code_category.ar_label
        ...
        if self.approved and (self.date_created is None):
            ...
            if self.delivery_type == self.COUPON:
                ...
                try:
                    ...
                    self.asset.save(self.parent_order.episode.activity.name + " - " + self.code_category.ar_label, File(output_file))
                    self.save()
